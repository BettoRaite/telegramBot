import {
  ...
  toSortedDates,
  fetchUserDataById} from '../lib/firebase.js';
...
if (false) {
  describe('sortDates', () => {
    describe(`sorts string dates arr in an ascending order
    by year, month, day in the format ["year-month-day"]`, () => {
      it('test1', () => {
...
if (false) {
  describe('fetchUserGroupId', () => {
    describe('should return user\'s group id', () => {
      describe('Good cases', () => {
        it(`Passing id of user that exists in db and has group id prop
        should return group id`, async () => {
          const userId = 'test';
          const groupId = 'test';
          await expect(fetchUserGroupId(userId))
              .to.eventually.deep.equal(groupId);
        });
      });
      describe('Bad cases', () => {
        it(`if user id is not a string
        should throw type error`, async () => {
          const expectedDescription = 'userId is expected to be a string';
          await expect(fetchUserGroupId())
              .to.be.rejectedWith(TypeError, expectedDescription);
        });
        it(`if user id is not found in db
        should throw syntax error`, async () => {
        ...
        it(`if fetched user doesn not have the groupId property
        should throw SyntaxError`, async () => {
...
if (false) {
  describe('fetchDataOnGroup', () => {
    describe('should return data of an existing group', () => {
      describe('Good cases', () => {
        it(`Passing id of an existing group in a db
        should return group data`, async () => {
      ...
      describe('Bad cases', () => {
        it(`If group id is not string type
        should throw type error`, async () => {
          const expectedDescription = 'groupId is expected to be a string';
          await expect(fetchDataOnGroup())
              .to.be.rejectedWith(TypeError, expectedDescription);
        });

        it(`Passing id of a group that does not exist in db
        should throw syntax error`, async () => {
          const uniqueId = String(uniqid());
