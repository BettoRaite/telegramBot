...
export const queueImageId = (userId, imageId) => {
	if (typeof imageId !== "string" || typeof userId !== "string") {
		errorHandler(
			new TypeError("image id must be of type string"),
			"users",
			"queueImageId",
		);
		return null;
...
export const queueCaption = (userId, caption) => {
	if (typeof caption !== "string" || typeof userId !== "string") {
		errorHandler(
			new TypeError("caption must be of type string"),
			"queueCaption",
			"user.js",
		);
		return null;
...
export const deleteUserAfter = (userId, timeSec) => {
	...
	if (!user) {
		errorHandler(
			new Error("Trying to reset undefined user"),
			"users",
			"deleteUserAfter",
		);
	}
	...
	if (!user.hasReset) {
		...
		setTimeout(async () => {
			if (
				getResetKey(userId) === resetKey &&
				!getUser(userId)?.isImageUploading
			) {
